...
**Queue choice — RabbitMQ.** The task asks specifically for a queueing mechanism. Both RabbitMQ and Redis Streams support native ack with at-least-once semantics (`BasicAck` / `XACK`) and concurrent consumers (competing consumers / consumer groups), so the differentiator isn't the ack primitive itself — it's the orchestration *around* ack. RabbitMQ gives you dead-letter exchanges, per-message TTL, automatic requeue on consumer death (the AMQP channel lifecycle does the work), and retry-count tracking as **queue-level configuration**. The same on Redis Streams is achievable but requires you to wire together `XAUTOCLAIM`, parallel "dead" streams, and your own periodic stuck-message sweeper. For a small service it's a wash; as the failure-handling story grows the RabbitMQ approach pays off faster — DLQ in particular is roughly one queue-declare line away (see Future improvements). An in-process `Channel<T>` would be the simplest possible answer but loses the durability and the visible decoupling the task is testing for.
...
2. `OrderConsumer` (a `BackgroundService`) subscribes to the queue with manual acks, `prefetchCount=5`, and `ConsumerDispatchConcurrency=5` on the connection factory — together those give up to 5 messages processed in parallel (prefetch alone isn't enough; the async consumer dispatches sequentially by default). Each message is handled in a freshly-scoped DI container so the `DbContext` lifecycle stays correct, and the shared `IModel` is protected by a small `lock` around `BasicAck` / `BasicNack` since the channel isn't thread-safe.
...
**Idempotency.** Each consumer attempt starts with an atomic `ExecuteUpdateAsync` claim that flips the order from `Pending → Processing` in one statement. If a duplicate delivery arrives (RabbitMQ redelivery, manual replay, a parallel worker), only one wins the claim — the others see `claimed == 0` and exit cleanly. Combined with manual acks this gives at-least-once delivery without double-processing. Closing the "saved but never published" window further would require the outbox pattern — see Future improvements.

**Failure handling.** If processing throws, the transaction rolls back, the order row is marked `Failed` with the exception message (via a separate, defensively-wrapped write), and the message is `BasicNack`'d *without requeue* — the outcome is already persisted on the order, so re-delivering would just spin the queue. See Known limitations and Future improvements for the DLQ story.
...
- **Dead-letter queue.** The consumer currently calls `BasicNack(requeue: false)` for messages it can't handle (deserialization errors, claim-time DB outages), which means those messages are silently dropped — `OrderConsumer.OnMessageReceived` has a `// In a real system this would route to a DLQ.` placeholder marking the spot. The fix is small: declare the queue with `x-dead-letter-exchange` arg pointing at a `orders.dlq` exchange + queue, and any nacked message lands there automatically. Then add a small admin endpoint to inspect / re-enqueue DLQ contents. Pairs naturally with the outbox above — together they give the service a full at-least-once delivery + retry-and-quarantine story.
